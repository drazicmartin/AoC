...
fn update_hasmap(
    n: i32,
    card_number: i32,
    mut card_multiplier: HashMap<i32, i32>,
) -> HashMap<i32, i32> {
    for i in 1..=(n) {
        *card_multiplier.entry(i + card_number).or_insert(0) +=
            *card_multiplier.get(&card_number).unwrap();
    }
...
fn main() {
    ...
    for i in (0..lines.len() - 1).collect::<Vec<usize>>() {
        ...
        *card_multiplier.entry(card_number).or_insert(0) += 1;
        ...
        sum += card_multiplier.get(&card_number).unwrap();
    }
